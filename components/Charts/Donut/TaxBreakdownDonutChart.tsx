...
export const TaxBreakdownDonutChart: React.FC<TaxBreakdownDonutChartProps> = ({ taxBreakdown }) => {
        ...
        {payload.map((entry:any, index:any) => (
          <li key={`item-${index}`} className="flex items-center">
            ...
            <span className="whitespace-nowrap">
              {entry.value} ({((entry.payload.value / totalTax) * 100).toFixed(1)}%)
            </span>
          </li>
        ))}
      </ul>
  ...
  return (
    <Card className="flex flex-col h-full">
      <CardHeader className="items-center pb-0">
        <CardTitle className="text-lg sm:text-xl text-center">{t('tax_breakdown')}</CardTitle>
        <CardDescription className="text-sm text-center px-2">
          {t('employee_tax_distribution')}
        </CardDescription>
      </CardHeader>
      ...
      <CardFooter className="flex-col gap-2 text-xs sm:text-sm mt-4 sm:mt-8">
        <div className="flex items-center gap-2 font-medium leading-none text-center">
          {t('total_tax')}: {formatSalary(totalTax)} 
